...
void perform(int n,int alpha){
	...
	for(i=0;i<n;i++){
		...
		for(k=0;k<alpha;k++){	
			...
			printf("nState-> %d for alphabet %c >> { ",i,alphabets[k]);
			int print;
			for(p=0;p<count;p++){
				print=1;
				for(int r=p-1;r>0;r--){
...
int main(){
	...
	scanf("%d",&n);
	printf("n Enter the no of alphabets: ");
	scanf("%d",&a);
	for(i=0;i<a;i++)
		scanf(" %c",&alphabets[i]);
	...
	printf("n Enter the no of transitions: ");
	scanf("%d", &t);
	printf("nEnter the state table:>>nSTATE ALPHABET STATEn");
	for(i=0;i<t;i++){
